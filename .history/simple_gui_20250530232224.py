...
if st.sidebar.button("Start Scheduler", key="start_scheduler_btn", disabled=scheduler_running_status, help="Khởi động tiến trình scheduler nền."):
    if not scheduler_running_status:
        # Attempt to clean up any potential stale stop request file before starting
        if os.path.exists(SCHEDULER_STOP_REQUEST_FILE):
            try:
                os.remove(SCHEDULER_STOP_REQUEST_FILE)
                st.toast("Dọn dẹp file stop request cũ trước khi khởi động scheduler.")
            except OSError:
                pass # Ignore if removal fails

        process_obj = run_script(SCHEDULER_SCRIPT, wait_for_completion=False)
        if process_obj:
            scheduler_confirmed_running = False
            pid_to_report = process_obj.pid # Initial PID from Popen
            
            # Wait for a few seconds for the scheduler to create its PID file and be verifiable
            max_wait_seconds = 7  # Max time to wait for scheduler to start
            poll_interval = 0.5   # How often to check
            elapsed_time = 0
            
            # Using st.toast for less intrusive feedback during the wait
            st.toast(f"Đang chờ scheduler (PID: {pid_to_report}) khởi động và tạo PID file...")

            while elapsed_time < max_wait_seconds:
                time.sleep(poll_interval)
                elapsed_time += poll_interval
                if is_scheduler_process_running(): # This checks PID file and process liveness
                    scheduler_confirmed_running = True
                    # Try to get the PID from the file, as it's the definitive one written by scheduler.py
                    try:
                        with open(SCHEDULER_PID_FILE, "r") as f_pid_read:
                            pid_from_file = int(f_pid_read.read().strip())
                        if psutil.pid_exists(pid_from_file): # Double check this PID
                           pid_to_report = pid_from_file
                    except (IOError, ValueError, psutil.Error):
                        pass # Stick with process_obj.pid if reading/checking file PID fails
                    break
            
            if scheduler_confirmed_running:
                st.session_state.last_action_message = f"INFO: Scheduler đã khởi động thành công (PID: {pid_to_report})."
            else:
                final_check_process_alive = process_obj.poll() is None # poll() is None if process is running
                if final_check_process_alive:
                    st.session_state.last_action_message = (
                        f"ERROR: Scheduler (PID: {process_obj.pid}) dường như đã khởi chạy "
                        f"nhưng không thể xác nhận trạng thái đầy đủ qua PID file sau {max_wait_seconds} giây. "
                        "Scheduler có thể đang khởi động chậm hoặc đã gặp lỗi trước khi tạo PID file."
                    )
                else:
                    st.session_state.last_action_message = (
                        f"ERROR: Khởi động Scheduler (PID: {process_obj.pid}) nhưng nó đã thoát "
                        f"trước khi có thể xác nhận trạng thái qua PID file (exit code: {process_obj.returncode})."
                    )
        else:
            st.session_state.last_action_message = "ERROR: Không thể khởi động tiến trình Scheduler (lỗi khi gọi run_script)."
        st.rerun()
