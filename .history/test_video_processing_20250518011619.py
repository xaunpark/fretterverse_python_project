...
from workflows.video_processor import process_videos_for_article
...
def run_video_test():
    ...
    test_run_context = RunContext(unique_run_id=UNIQUE_RUN_ID_VID_TEST)
    ...
    with mock.patch('workflows.video_processor.call_openai_chat', side_effect=mock_openai_for_video), \
         ...
         mock.patch('workflows.video_processor.perform_search', side_effect=mock_youtube_search): # Mock perform_search nếu dùng
        ...
        final_video_results = process_videos_for_article(
            ...
            config=APP_CONFIG
            # unique_run_id không còn cần thiết ở đây nếu RunContext đã có
        )
