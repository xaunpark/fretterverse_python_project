...
from workflows.image_processor import process_images_for_article, _get_redis_keys # Import hàm cần test
from utils.redis_handler import RedisHandler # Để dọn dẹp Redis sau test (nếu dùng Redis thật)
...
try:
    ...
    APP_CONFIG['GOOGLE_SEARCH_NUM_RESULTS_IMAGES'] = 3 # Lấy ít ảnh hơn
...
def mock_openai_chat_responses(*args, **kwargs):
    # Lấy nội dung của message đầu tiên trong list messages
    # Thường thì prompt_messages sẽ là list chỉ có 1 phần tử dict {"role": "user", "content": "..."}
    # Nhưng để an toàn, kiểm tra trước khi truy cập
    prompt_messages_list = kwargs.get('prompt_messages', [])
    if not prompt_messages_list or not isinstance(prompt_messages_list[0], dict):
        ...
        return {} if kwargs.get('is_json_output', False) else None
        
    prompt_content = prompt_messages_list[0].get('content', '').lower() # Chuyển sang lowercase để dễ so sánh
    is_json = kwargs.get('is_json_output', False)
    
    logger.debug(f"Mock OpenAI Chat called. JSON output: {is_json}. Prompt starts with: {prompt_content[:150]}") # Log nhiều hơn một chút

    # Điều kiện để nhận diện prompt tạo image keyword
    # Dựa vào nội dung của image_prompts.GENERATE_IMAGE_SEARCH_KEYWORD_PROMPT
    if "identify the central theme" in prompt_content and "recommend for a relevant image search" in prompt_content:
        ...
        # Trả về một keyword giả dựa trên section_name trong prompt
        if "spruce tops" in prompt_content: # Kiểm tra section_name có trong prompt không
            return "acoustic guitar spruce top"
        elif "mahogany's warmth" in prompt_content:
            return "mahogany guitar body"
        elif "another content section" in prompt_content: # Thêm case cho section này
            return "cool guitar photo" 
        return "generic guitar image" 
    
    elif "which image best visually represents" in prompt_content and is_json:
        logger.info("Mock OpenAI: Matched prompt for choosing best image URL.")
        # Giả lập việc chọn ảnh.
        options_str_from_prompt = prompt_messages_list[0].get('content', '') # Lấy lại prompt đầy đủ
        ...
        if "http://example.com/cool_guitar.jpg" in options_str_from_prompt: # Thêm case cho ảnh của "cool guitar photo"
            return {"imageURL": "http://example.com/cool_guitar.jpg", "imageDes": "A very cool guitar"}
        if "http://example.com/generic1.jpg" in options_str_from_prompt: # Giữ lại fallback
    ...
    return {} if is_json else None # Trả về dict rỗng cho JSON hoặc None cho text
...
# --- Hàm dọn dẹp Redis (quan trọng cho test) ---
def cleanup_redis_for_test(config, run_id):
    try:
        redis_h = RedisHandler(config=config)
        if redis_h.is_connected():
            keys_to_delete = _get_redis_keys(config, run_id)
            for key_name, key_pattern_val in keys_to_delete.items():
                if key_name == "current_image_search_results_prefix":
                    # Xóa các key theo pattern
                    # Cẩn thận: redis_h.r.keys() có thể chậm trên DB lớn
                    # Trong test với DB riêng thì ok
                    for section_idx in range(len(SAMPLE_SECTIONS_DATA)):
                        actual_key = f"{key_pattern_val}{section_idx}"
                        redis_h.delete_key(actual_key)
                else:
                    redis_h.delete_key(key_pattern_val)
            logger.info(f"Cleaned up Redis keys for run_id: {run_id}")
    except Exception as e:
...
def run_test():
    ...
    with mock.patch('workflows.image_processor.call_openai_chat', side_effect=mock_openai_chat_responses), \
        ...
        final_image_results = process_images_for_article(
            ...
            config=APP_CONFIG, # Truyền APP_CONFIG đã load
            unique_run_id=UNIQUE_RUN_ID_TEST
        )
...
if __name__ == "__main__":
    try:
        ...
        # Dọn dẹp Redis sau khi test, kể cả khi có lỗi
        logger.info("--- Cleaning up Test Data ---")
        cleanup_redis_for_test(APP_CONFIG, UNIQUE_RUN_ID_TEST)
