...
def load_app_config(site_name=None):
    ...
    if hasattr(st, 'secrets') and st.secrets and st.secrets.get("GOOGLE_APPLICATION_CREDENTIALS_JSON_STR"):
        creds_json_content_str = st.secrets.get("GOOGLE_APPLICATION_CREDENTIALS_JSON_STR")
        logger.info("Found GOOGLE_APPLICATION_CREDENTIALS_JSON_STR in Streamlit secrets. Attempting to process.")
        ...
        try:
            # Cố gắng parse chuỗi từ secrets như một JSON object
            # Sau đó dump lại vào file để đảm bảo JSON valid và escaping đúng
            parsed_creds = json.loads(creds_json_content_str)
            with open(temp_creds_path, "w") as f:
                json.dump(parsed_creds, f, indent=2) # Ghi lại dưới dạng JSON chuẩn
            config['GOOGLE_APPLICATION_CREDENTIALS'] = temp_creds_path # Ghi đè đường dẫn
            logger.info(f"Successfully created temporary service account file via json.dump at {temp_creds_path}")
        except json.JSONDecodeError as jde:
            logger.error(f"Failed to parse GOOGLE_APPLICATION_CREDENTIALS_JSON_STR from secrets as JSON: {jde}. "
                         f"Problematic string (first 200 chars): '{creds_json_content_str[:200]}...'")
            logger.warning(f"Falling back to writing the raw string from secrets to {temp_creds_path}. This might lead to auth errors if the string is not valid JSON for the auth library.")
            # Fallback: ghi chuỗi gốc, có thể vẫn gây lỗi nếu chuỗi gốc không phải là JSON hợp lệ
            try:
                with open(temp_creds_path, "w") as f:
                    f.write(creds_json_content_str)
                config['GOOGLE_APPLICATION_CREDENTIALS'] = temp_creds_path
            except Exception as e_raw_write:
                logger.error(f"Error writing raw GOOGLE_APPLICATION_CREDENTIALS_JSON_STR to temp file: {e_raw_write}")
