...
def load_app_config(site_name=None):
    ...
    # Ensure schedule settings have valid defaults and types
    if not isinstance(config.get('SCHEDULE_ENABLED'), bool):
        logger.warning(f"SCHEDULE_ENABLED ('{config.get('SCHEDULE_ENABLED')}') is not a valid boolean. Defaulting to False.")
