...
def initialize_app(site_name_arg=None): # Thêm site_name_arg
    ...
    try:
        ...
        # Sử dụng một tên file log cụ thể cho orchestrator, hoặc lấy từ config nếu muốn
        log_file_path_from_config = APP_CONFIG.get('ORCHESTRATOR_LOG_FILE_PATH', 'logs/orchestrator.log')
        current_log_file_path = log_file_path_from_config if log_to_file_flag else None
        
        # Gán trực tiếp logger được trả về từ setup_logging cho biến global
        ...
        # logger = logging.getLogger(__name__) # Dòng này không còn cần thiết nếu logger global là root logger đã cấu hình
        logger.info("Application initialized successfully.")
        return True
    except ValueError as e: 
        print(f"CRITICAL: Configuration Error - {e}. Application cannot start.")
        if logger: logger.critical(f"Configuration Error - {e}. Application cannot start.", exc_info=True) # logger có thể vẫn là None ở đây nếu lỗi xảy ra trong load_app_config
        return False
...
def main():
    ...
    if not initialize_app(site_name_arg=args.site):
        # Đảm bảo print ra console vì logger có thể chưa được khởi tạo đầy đủ
        print(f"CRITICAL: Application initialization failed for site '{args.site}'. Orchestrator cannot continue.")
        sys.exit(1) # Quan trọng: Thoát với mã lỗi khác 0
