...
from unittest import mock # Thư viện mock rất quan trọng
...
def mock_openai_chat_responses(*args, **kwargs):
    prompt_content = kwargs.get('prompt_messages', [{}])[0].get('content', '').lower()
    is_json = kwargs.get('is_json_output', False)
    
    logger.debug(f"Mock OpenAI Chat called. JSON output: {is_json}. Prompt starts with: {prompt_content[:100]}")

    if "image keyword" in prompt_content or "recommend for a relevant image search" in prompt_content:
        # Trả về một keyword giả
        if "spruce" in prompt_content:
            return "acoustic guitar spruce top"
        elif "mahogany" in prompt_content:
            return "mahogany guitar body"
        return "generic guitar image" # Default keyword
    
    elif "which image best visually represents" in prompt_content and is_json:
        # Giả lập việc chọn ảnh. Lấy ảnh đầu tiên từ options nếu có.
        options_str = kwargs.get('prompt_messages', [{}])[0].get('content', '')
        # Đơn giản là chọn ảnh đầu tiên nếu có
        if "1. Image Description:" in options_str:
            if "http://example.com/spruce.jpg" in options_str:
                 return {"imageURL": "http://example.com/spruce.jpg", "imageDes": "A beautiful spruce top guitar"}
            if "http://example.com/mahogany.jpg" in options_str:
                 return {"imageURL": "http://example.com/mahogany.jpg", "imageDes": "Warm mahogany guitar"}
            if "http://example.com/generic1.jpg" in options_str:
                 return {"imageURL": "http://example.com/generic1.jpg", "imageDes": "A generic guitar"}
        return {"imageURL": None, "imageDes": "AI could not select an image"} # Nếu không có options
    
    logger.warning(f"Unhandled mock OpenAI chat prompt: {prompt_content[:100]}")
