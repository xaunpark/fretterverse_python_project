...
def main_delete():
    ...
    # Thay đổi để đọc trực tiếp từ site_config
    gsheet_spreadsheet_id = site_config.get('GSHEET_SPREADSHEET_ID')
    if not gsheet_spreadsheet_id:
        logger.critical(f"Delete Script: 'GSHEET_SPREADSHEET_ID' not found in site_config for '{site_profile_name}'. Exiting.")
        return
    
    # Lấy tên sheet "Delete": ưu tiên từ site_config, nếu không có thì từ APP_CONFIG, cuối cùng là 'Delete'
    # Giả sử bạn có thể thêm GSHEET_DELETE_SHEET_NAME vào site_config.json nếu cần
    gsheet_delete_sheet_name = site_config.get('GSHEET_DELETE_SHEET_NAME', APP_CONFIG.get('GSHEET_DELETE_SHEET_NAME', 'Delete'))

    # Pinecone index name: ưu tiên từ site_config, nếu không có thì PineconeHandler sẽ dùng từ APP_CONFIG
    # Thay đổi để đọc trực tiếp từ site_config
    pinecone_index_name_override = site_config.get('PINECONE_INDEX_NAME') # Sẽ là None nếu không có trong site_config
