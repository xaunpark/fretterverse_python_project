...
from workflows.external_links_processor import process_external_links_for_article, _extract_text_from_html
...
def run_ext_links_test():
    ...
    # Khởi tạo RunContext cho test
    test_run_context = RunContext(unique_run_id=UNIQUE_RUN_ID_EXT_TEST)
    # Giả lập link này đã có trong RunContext
    test_run_context.used_external_links.add("http://alreadyusedlink.com/info")

    with mock.patch('workflows.external_links_processor.call_openai_chat', side_effect=mock_openai_for_ext_links), \
         mock.patch('workflows.external_links_processor.perform_search', side_effect=mock_google_search_for_ext_links): # Sửa google_search thành perform_search
