...
from utils.html_utils import markdown_to_html_advanced
        ...
        if prompt_for_llm == content_prompts.SAY_I_LOVE_YOU_PROMPT:
            ...
            if llm_response:
                ...
                if "i love you" in llm_response.lower() and len(llm_response) < 20:
                    ...
                    # Heuristic đơn giản: nếu có dấu ** hoặc * (không phải trong tag HTML) thì có thể là Markdown
                    if ("**" in llm_response or "*" in llm_response) and not ("<strong" in llm_response or "<em" in llm_response):
                        logger.info(f"LLM response for section '{section_copy.get('sectionName')}' seems to be Markdown. Converting to HTML.")
                        # Dùng hàm advanced nếu bạn đã cài thư viện markdown, nếu không dùng basic
                        # Hoặc thêm một cờ config để quyết định có convert không
                        html_content = markdown_to_html_advanced(llm_response)
                    else:
                        html_content = llm_response # Giả sử đã là HTML
            else:
