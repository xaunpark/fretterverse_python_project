...
def check_keyword_uniqueness_and_upsert(keyword, config, gsheet_handler, pinecone_handler, 
    ...
    if is_unique:
        ...
        upsert_result = pinecone_handler.upsert_vectors(
            ...
            # namespace=None, # Có thể thêm nếu cần
            # batch_size=config.get('PINECONE_UPSERT_BATCH_SIZE', 100) # Có thể thêm nếu cần
        )
        if upsert_result and upsert_result.upserted_count > 0:
            ...
            gsheet_update_data[config.get('GSHEET_UNIQUE_COLUMN')] = "error_pinecone_upsert" # Ghi nhận lỗi vào GSheet
            is_unique = False # Coi như không unique nếu không upsert được (quan trọng)
    else: # Not unique
