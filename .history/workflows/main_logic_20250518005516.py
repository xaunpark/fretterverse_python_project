...
def process_sub_workflows_step(sections_with_initial_content, article_meta, config):
    """
    ...
    article_meta: Chứa article_title và các meta khác.
    ...
    if not sections_after_external_links:
        ...
        # Cập nhật lại key 'html_content' trong một cấu trúc mới để trả về
        # sections_with_initial_content sẽ được dùng cho image/video processors (chứa cấu trúc section gốc)
        # sections_final_content_structure sẽ chứa HTML đã được cập nhật bởi external links
        ...
        for i, sec in enumerate(sections_final_content_structure):
            sections_final_content_structure[i]['html_content'] = temp_dict_after_ext_links.get(
                sec['sectionIndex'], 
                sec.get('html_content', '') # Fallback nếu sectionIndex không tìm thấy (không nên xảy ra)
            )
