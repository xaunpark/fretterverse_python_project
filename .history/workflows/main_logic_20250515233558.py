...
import html
from utils.api_clients import call_openai_chat, google_search, call_openai_embeddings
from utils.google_sheets_handler import GoogleSheetsHandler
from utils.redis_handler import RedisHandler
from utils.pinecone_handler import PineconeHandler
from prompts import main_prompts
from prompts import content_prompts
...
def _determine_category_id(article_title, keyword_analysis_data, config, gsheet_handler, wp_auth):
    """
    ...
    # 1. Lấy danh sách category hiện có từ WordPress để đưa vào prompt
    # Hàm này cần được viết trong api_clients.py hoặc một utils riêng cho WordPress
    # Giả sử get_wp_categories trả về list các dict {'id': ..., 'name': ..., 'parent': ...}
    # Hoặc bạn có thể hardcode/cache danh sách này nếu nó ít thay đổi.
    # Trong n8n, bạn lấy categoryList từ Google Sheet. Ở đây, ta có thể gọi WP API.
    
    # Lấy category list từ GSheet handler (như trong n8n logic của bạn)
    # Cần có hàm get_all_categories_structured trong gsheet_handler hoặc ở đây
    # Hoặc gọi trực tiếp WordPress API:
    wp_categories_raw = []
    try:
        # Giả sử hàm này đã được định nghĩa trong api_clients.py
        from utils.api_clients import get_wp_categories 
        wp_categories_raw = get_wp_categories(config.get('WP_BASE_URL'), wp_auth[0], wp_auth[1], params={'per_page': 100, 'orderby': 'count', 'order': 'desc'})
    except Exception as e:
        logger.error(f"Failed to fetch WordPress categories: {e}")

    category_structure_str = "Available categories:\n"
    if wp_categories_raw and isinstance(wp_categories_raw, list):
        # Tạo chuỗi mô tả category (Parent: Child1, Child2)
        parents = {c['id']: c['name'] for c in wp_categories_raw if c.get('parent') == 0}
        children_map = {}
        for c in wp_categories_raw:
            if c.get('parent') != 0 and c.get('parent') in parents:
                if c['parent'] not in children_map:
                    children_map[c['parent']] = []
                children_map[c['parent']].append(c['name'])
    ...
    prompt = main_prompts.RECOMMEND_CATEGORY_PROMPT.format(
        ...
        keyword=article_meta.get("original_keyword", article_title), # Sử dụng keyword gốc
    ...
    if not category_recommendation or not isinstance(category_recommendation, dict):
        ...
        return config.get('DEFAULT_CATEGORY_ID', 86) # Fallback

    is_new_category = category_recommendation.get('isNew', 'no').lower() == 'yes'
    recommended_cat_name = category_recommendation.get('recommendation', {}).get('category')
    suggested_new_name = category_recommendation.get('suggestedName')

    final_category_id = None
    final_category_name = None

    if is_new_category and suggested_new_name:
        logger.info(f"LLM suggests creating a new category: '{suggested_new_name}'")
        # Gọi API tạo category mới trên WordPress
        new_cat_response = create_wp_category(
            base_url=config.get('WP_BASE_URL'),
            auth_user=wp_auth[0], auth_pass=wp_auth[1],
            name=suggested_new_name
            # parent_id=0 # Mặc định tạo ở cấp gốc, có thể thêm logic chọn parent
        )
        if new_cat_response and new_cat_response.get('id'):
            final_category_id = new_cat_response.get('id')
            final_category_name = suggested_new_name
            logger.info(f"Successfully created new WordPress category '{final_category_name}' with ID: {final_category_id}")
            # Có thể cần cập nhật lại gsheet_handler.category_data nếu bạn cache nó
        else:
            logger.error(f"Failed to create new category '{suggested_new_name}' on WordPress. Response: {new_cat_response}")
            final_category_id = config.get('DEFAULT_CATEGORY_ID', 86)
            final_category_name = "Default Fallback Category"
    elif recommended_cat_name:
        # Tìm ID của category được khuyến nghị từ danh sách category đã lấy
        found_cat = None
        if wp_categories_raw and isinstance(wp_categories_raw, list):
            for cat in wp_categories_raw:
                # So sánh không phân biệt hoa thường và bỏ &
                if html.unescape(cat.get('name','').lower()) == html.unescape(recommended_cat_name.lower()):
                    found_cat = cat
                    break
        if found_cat:
            final_category_id = found_cat.get('id')
            final_category_name = found_cat.get('name')
            logger.info(f"Using existing category: '{final_category_name}' (ID: {final_category_id})")
        else:
            logger.warning(f"Recommended category '{recommended_cat_name}' not found in existing WordPress categories. Falling back to default.")
            final_category_id = config.get('DEFAULT_CATEGORY_ID', 86)
            final_category_name = "Default Fallback Category"
    else:
...
def _php_serialize_internal_link_keywords(keywords_list):
    ...
    Ví dụ: ['keyword1', 'keyword1 variant'] -> 'a:2:{i:0;s:8:"keyword1";i:1;s:16:"keyword1 variant";}'
    """
    if not keywords_list or not isinstance(keywords_list, list):
        return 'a:0:{}' # Mảng rỗng

    serialized_parts = []
    for i, keyword in enumerate(keywords_list):
        # Xử lý ký tự đặc biệt trong PHP string (đơn giản hóa, chỉ ' và \)
        # ILJ có thể có cách xử lý riêng, cần kiểm tra
        escaped_keyword = keyword.replace('\\', '\\\\').replace("'", "\\'")
        serialized_parts.append(f'i:{i};s:{len(keyword.encode("utf-8"))}:"{escaped_keyword}";')
        # Dùng len(keyword.encode("utf-8")) để đếm byte chính xác cho ký tự đa byte

    return f'a:{len(keywords_list)}:{{{"".join(serialized_parts)}}}'


def finalize_and_publish_article_step(
    full_article_html, article_meta, preparation_data,
    config, gsheet_handler, db_handler, # Thêm db_handler (MySQL)
    unique_run_id # Để tải ảnh feature nếu cần
    ):
    """
    ...
    dalle_prompt_description = call_openai_chat(
        prompt_messages=[{"role": "user", "content": image_prompts.GENERATE_DALLE_FEATURED_IMAGE_DESCRIPTION_PROMPT.format(
            article_title_raw=article_meta.get('title')
        )}],
        model_name=config.get('DEFAULT_OPENAI_CHAT_MODEL'), # Model thường cho việc tạo prompt
        api_key=config.get('OPENAI_API_KEY')
    )
    ...
    if dalle_prompt_description:
        ...
        featured_image_url_from_dalle = call_openai_dalle(
            ...
            size=config.get('FEATURED_IMAGE_SIZE', "1024x1024"), # Kích thước từ config
            api_key=config.get('OPENAI_API_KEY'),
        ...
        if featured_image_url_from_dalle:
            ...
            try:
                ...
                resized_featured_image_binary = resize_image(
                    ...
                    # height=None, # Giữ aspect ratio
                    output_format='JPEG', # Luôn là JPEG cho featured image
                    quality=85
                )
                if resized_featured_image_binary:
                    # Tạo filename cho featured image
                    slug_for_filename = article_meta.get('slug', 'featured').replace('-', '_')
                    featured_filename = f"{slug_for_filename}_featured_image.jpg"
                    
                    wp_media_resp = upload_wp_media(
    ...
    keyword_analysis = preparation_data.get("keyword_analysis", {}) # Lấy từ Bước 1
    category_id_for_post = _determine_category_id(article_meta.get('title'), keyword_analysis, config, gsheet_handler, wp_auth)
    ...
    from utils.api_clients import create_wp_post, update_wp_post # Đảm bảo import
    ...
    post_payload = {
        ...
        "categories": [category_id_for_post], # Phải là list
        "author": article_meta.get('chosen_author_id', config.get('DEFAULT_AUTHOR_ID')),
        "excerpt": article_meta.get('description') # SEO description dùng làm excerpt
        # featured_media sẽ được cập nhật sau nếu post được tạo thành công
    }
    
    created_post_response = create_wp_post(
        base_url=wp_base_url, auth_user=wp_user, auth_pass=wp_pass,
        **post_payload # Giải nén dict thành keyword arguments
    )
    ...
    if created_post_response and created_post_response.get('id'):
        ...
        # Cập nhật featured image nếu có
        if featured_image_wp_id:
            time.sleep(2) # Chờ một chút để post được lưu hoàn toàn trước khi update
        ...
        # Quyết định xem có dừng ở đây không. Nếu không có post_id, các bước sau sẽ thất bại.
        return False # Báo hiệu thất bại

    # 4. Cập nhật Google Sheet
    if gsheet_handler and post_id:
        gsheet_update_data = {
            config.get('GSHEET_USED_COLUMN'): "1",
            config.get('GSHEET_POST_TITLE_COLUMN'): article_meta.get('title'),
            config.get('GSHEET_POST_ID_COLUMN'): str(post_id), # Đảm bảo là string
            config.get('GSHEET_POST_URL_COLUMN'): post_url
            # Cột 'Uniqe' và 'Suitable' đã được cập nhật ở Bước 1
        }
        ...
        if success_gsheet:
            ...
            # Đây không phải lỗi nghiêm trọng để dừng toàn bộ, chỉ log lại
    ...
    if db_handler and post_id:
        ...
        ilj_keywords_response = call_openai_chat(
            ...
            model_name=config.get('DEFAULT_OPENAI_CHAT_MODEL'),
            api_key=config.get('OPENAI_API_KEY'),
            is_json_output=True # Prompt yêu cầu JSON list
        )
        if ilj_keywords_response and isinstance(ilj_keywords_response, list):
            ...
            # Xóa meta_key trùng lặp trước (nếu có)
            delete_query = """
            ...
            """
            db_handler.execute_query(delete_query, params=(post_id, post_id)) # is_transaction=False để tự commit
            
            # Insert hoặc Update
            upsert_query = """
            INSERT INTO wp_postmeta (post_id, meta_key, meta_value) 
            VALUES (%s, 'ilj_linkdefinition', %s)
            ON DUPLICATE KEY UPDATE meta_value = VALUES(meta_value);
            """
            # Cần đảm bảo rằng nếu meta_key 'ilj_linkdefinition' đã tồn tại cho post_id, 
            # thì ON DUPLICATE KEY UPDATE sẽ hoạt động đúng.
            # Thường thì (post_id, meta_key) là unique key hoặc primary key cho update.
            # Nếu không, bạn có thể cần một lệnh UPDATE riêng nếu SELECT tìm thấy key.
            # Hoặc đảm bảo logic XÓA rồi INSERT. Logic hiện tại của bạn là XÓA DUPLICATE rồi INSERT/UPDATE.
            
            # Để an toàn hơn, có thể xóa tất cả ilj_linkdefinition rồi insert mới:
            # db_handler.execute_query("DELETE FROM wp_postmeta WHERE post_id = %s AND meta_key = 'ilj_linkdefinition'", params=(post_id,))
            # insert_query_simple = "INSERT INTO wp_postmeta (post_id, meta_key, meta_value) VALUES (%s, 'ilj_linkdefinition', %s)"
            # result_ilj_db = db_handler.execute_query(insert_query_simple, params=(post_id, serialized_php_string))

            result_ilj_db = db_handler.execute_query(upsert_query, params=(post_id, serialized_php_string))

            if result_ilj_db is not None: # execute_query trả về rowcount
                logger.info(f"Successfully updated/inserted Internal Link Juicer data for post ID {post_id}. Rows affected/status: {result_ilj_db}")
            else:
                logger.error(f"Failed to update/insert Internal Link Juicer data for post ID {post_id}.")
        else:
            logger.warning(f"Could not generate ILJ keywords or invalid format from LLM. Response: {ilj_keywords_response}")
    else:
        logger.warning("MySQL handler not available or no post_id. Skipping Internal Link Juicer.")

    # 6. (Tùy chọn) Google Indexing API
    if config.get('ENABLE_GOOGLE_INDEXING', False) and post_url:
        ...
        # success_indexing = submit_to_google_indexing(post_url, config)
        # if success_indexing:
        #     logger.info("Successfully submitted URL to Google Indexing API.")
        # else:
        #     logger.error("Failed to submit URL to Google Indexing API.")
        pass # Placeholder

    # 7. (Tùy chọn) Chia sẻ mạng xã hội
    if config.get('ENABLE_SOCIAL_SHARING', False) and post_url:
        logger.info(f"Initiating social sharing for: {post_url}")
        # Gọi các hàm/workflow chia sẻ mạng xã hội ở đây
        pass # Placeholder

    logger.info(f"--- Step 6 (Finalize and Publish) completed for post ID: {post_id} ---")
    return {"post_id": post_id, "post_url": post_url, "status": "published"} if post_id else False