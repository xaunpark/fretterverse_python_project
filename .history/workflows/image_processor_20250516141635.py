...
from utils.redis_handler import RedisHandler # Giả sử bạn dùng class RedisHandler
...
def _parse_image_items_from_google(google_items_list):
    """Chuyển đổi kết quả từ Google Image Search thành list các dict {imageUrl, imageDes}."""
    if not google_items_list:
        return []
    images_data = []
    for item in google_items_list:
        image_url = item.get('link')
        # Google Image Search API không trả về 'snippet' trực tiếp như web search cho từng ảnh
        # 'title' có thể dùng làm description, hoặc bạn có thể lấy 'image.contextLink' rồi crawl snippet (phức tạp)
        # Trong n8n, bạn dùng item.snippet. Có vẻ như bạn đang dùng custom search API cho image
        # và nó trả về 'snippet' cho mỗi 'item'. Nếu API bạn dùng trả về snippet:
        image_des = item.get('snippet', item.get('title', 'No description available'))
        if image_url:
            images_data.append({'imageUrl': image_url, 'imageDes': image_des})
    return images_data

def _filter_image_search_results(image_search_list, failed_urls_list):
    """Lọc bỏ các URL đã thất bại khỏi danh sách tìm kiếm ảnh."""
...
def process_single_section_image(section_data, article_title, parent_section_name_for_subchapter,
                                 ...
                                 wp_auth_tuple, unique_run_id, section_index_for_redis_key):
    """
    ...
    if not image_search_results:
        ...
        if not cx_id_shared:
            logger.error("GOOGLE_CX_ID not configured for image search in image_processor.")
            # Xử lý lỗi, ví dụ trả về không tìm thấy ảnh
            return {"url": "config_error_no_cx_id", "index": section_data.get('sectionIndex')}
                
        google_items = google_search(
            query=encoded_keyword,
            api_key=google_api_key, # Đã được truyền vào hoặc lấy từ config
            cx_id=cx_id_shared,     # TRUYỀN CX_ID CHUNG
            search_type='image',    # QUAN TRỌNG: Đặt searchType là 'image'
            num_results=config.get('GOOGLE_SEARCH_NUM_RESULTS_IMAGES', 10),
            imgSize="large" 
        )
        image_search_results = _parse_image_items_from_google(google_items)
    ...
    for attempt in range(max_selection_attempts):
        ...
        for i, img_data in enumerate(current_image_options_list):
            ...
            options_parts.append(f"{i+1}. Image Description: {desc}, imageURL: {url}")
        image_options_str = "\n".join(options_parts)
...
def process_images_for_article(sections_data_list, article_title, config, unique_run_id):
    """
    ...
    unique_run_id: ID duy nhất cho lần chạy xử lý bài viết này, dùng để tạo Redis key.
    """
    logger.info(f"Starting image processing for article: '{article_title}' with run_id: {unique_run_id}")
    ...
    if not all([openai_api_key, google_api_key, wp_user, wp_pass, config.get('GOOGLE_IMAGES_CX_ID')]):
        logger.error("Missing critical API keys or WordPress credentials in config for image processing.")
    ...
    for i, section in enumerate(sections_data_list):
        ...
        current_global_image_array = redis_handler.get_value(redis_keys['image_array']) or []
        ...
        final_image_array_from_redis.append(current_image_data_for_section)


    # Sắp xếp lại final_image_array_from_redis theo 'index' nếu cần thiết (nếu xử lý song song)
    # Nếu xử lý tuần tự thì nó đã theo thứ tự.
    # Loại bỏ trùng lặp theo index nếu có (hiếm khi xảy ra với logic tuần tự)
    # unique_images_by_index = {img['index']: img for img in final_image_array_from_redis}
    # sorted_unique_images = sorted(unique_images_by_index.values(), key=lambda x: x['index'])

    # Lưu final_image_array_from_redis (đã được build ở Python) vào Redis một lần
    # Điều này đơn giản hơn việc đọc/ghi Redis liên tục cho image_array chính.
    redis_handler.set_value(redis_keys['image_array'], final_image_array_from_redis) # Lưu kết quả cuối cùng vào redis
    
    logger.info(f"Finished image processing for article '{article_title}'. Final image data for this run (before potential global sort/merge): {final_image_array_from_redis}")
