...
def analyze_and_prepare_keyword(keyword_to_process, config, gsheet_handler, pinecone_handler):
    ...
    # chosen_author giờ sẽ luôn là một dict (hoặc default dict), không nên là None nếu get_default_author hoạt động đúng
    if not (chosen_author and 'ID' in chosen_author and 'name' in chosen_author and 'info' in chosen_author) :
        logger.critical(f"CRITICAL: Failed to obtain valid author data (even default) for keyword '{keyword_to_process}'. Cannot proceed with content generation.")
        # Đây là lỗi nghiêm trọng nếu ngay cả author mặc định cũng không được tạo đúng
        return None 
...
def finalize_and_publish_article_step(
    ...
    post_payload = {
        ...
        "categories_ids": [category_id_for_post] if category_id_for_post else [config.get('DEFAULT_CATEGORY_ID', 86)], # SỬA KEY Ở ĐÂY
        "author_id": article_meta.get('chosen_author_id', config.get('DEFAULT_AUTHOR_ID')), # SỬA KEY Ở ĐÂY
        "excerpt": article_meta.get('description')
        # featured_media_id sẽ được truyền riêng nếu cần, hoặc thêm vào payload nếu hàm create_wp_post hỗ trợ
    }
    ...
    created_post_response = create_wp_post(
        ...
        auth_user=wp_user, 
        auth_pass=wp_pass,
        # Truyền các tham số theo đúng tên trong định nghĩa hàm create_wp_post
        title=post_payload["title"],
        content=post_payload["content"],
        slug=post_payload["slug"],
        status=post_payload["status"],
        categories_ids=post_payload["categories_ids"], # TRUYỀN ĐÚNG TÊN
        author_id=post_payload["author_id"],           # TRUYỀN ĐÚNG TÊN
        excerpt=post_payload["excerpt"]
        # featured_media_id=featured_image_wp_id # Nếu hàm create_wp_post hỗ trợ
    )
