...
def analyze_and_prepare_keyword(keyword_to_process, config, gsheet_handler, pinecone_handler):
    ...
    logger.info(f"Starting Step 1: Analyze and Prepare Keyword for '{keyword_to_process}'")

    # 0. Kiểm tra tính phù hợp
    if not check_keyword_suitability(keyword_to_process, config, gsheet_handler):
        logger.warning(f"Process stopped for Step 1: Keyword '{keyword_to_process}' is not suitable.")
        return None
    preparation_data["is_suitable"] = True
    logger.info(f"Keyword '{keyword_to_process}' passed suitability check.")

    # 1. Kiểm tra tính duy nhất và upsert
    is_unique = check_keyword_uniqueness_and_upsert(keyword_to_process, config, gsheet_handler, pinecone_handler)
    if not is_unique:
        logger.warning(f"Process stopped for Step 1: Keyword '{keyword_to_process}' is not unique or error during uniqueness check.")
        return None
    preparation_data["is_unique"] = True
    logger.info(f"Keyword '{keyword_to_process}' passed uniqueness check and/or was upserted.")

    # 2. Chọn tác giả
    # Lấy danh sách author personas từ config
    author_personas_list = config.get('AUTHOR_PERSONAS', []) 
    if not author_personas_list:
         logger.warning("AUTHOR_PERSONAS not found or empty in config. Author selection might use default.")
         
    # Truyền list Python vào hàm choose_author_for_topic
    chosen_author = choose_author_for_topic(keyword_to_process, author_personas_list, config) 
    if not (chosen_author and 'ID' in chosen_author and chosen_author.get('ID') is not None): # Kiểm tra ID không rỗng
        logger.critical(f"CRITICAL: Failed to obtain valid author data (ID is missing or None) for keyword '{keyword_to_process}'. Cannot proceed.")
        return None 
    preparation_data["chosen_author"] = chosen_author
    logger.info(f"Author for '{keyword_to_process}': {chosen_author.get('name')}, ID: {chosen_author.get('ID')}") # Log ID

    # 3. Lấy SERP data
    serp_data_str = get_serp_data_for_keyword(keyword_to_process, config)
    # Không dừng nếu không có SERP, chỉ cảnh báo và ghi nhận
    ...
    keyword_analysis_result = analyze_serp_and_keyword(keyword_to_process, serp_data_str, config) # Đổi tên biến để tránh nhầm lẫn
    if not keyword_analysis_result:
        logger.error(f"Failed to analyze keyword and SERP for '{keyword_to_process}'. Cannot proceed with outline generation.")
        return None 
