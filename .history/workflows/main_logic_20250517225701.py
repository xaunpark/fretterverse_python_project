...
def _generate_comparison_table_if_needed(article_meta, processed_sections_list, config):
    ...
    try:
        ...
        logger.debug(f"After html.unescape (first 50 chars): {processed_html[:50]}")

        # Fallback: Manually unescape common table-related entities if html.unescape might have missed them
        # This addresses the issue observed in the log where processed_html still contained &lt;
        if "&lt;" in processed_html or "&gt;" in processed_html:
            logger.warning("Manually unescaping &lt; and &gt; as html.unescape might not have fully processed them for table tags.")
            processed_html = processed_html.replace("&lt;", "<").replace("&gt;", ">")
            # It's good practice to also handle &amp; if it might lead to &amp;lt; becoming &lt; then not unescaped.
            # However, the log suggests &lt; was the direct issue.
            logger.debug(f"After manual replace (first 50 chars): {processed_html[:50]}")
