...
def get_serp_data_for_keyword(keyword, config):
    ...
    cx_id_serp = config.get('GOOGLE_CX_ID_FOR_SERP_ANALYSIS')
    if not cx_id_serp:
        logger.error("GOOGLE_CX_ID_FOR_SERP_ANALYSIS not configured. Cannot fetch SERP data.")
        return None    
    try:
...
def analyze_and_prepare_keyword(keyword_to_process: str, 
    ...
    # --- 3. Lấy SERP data ---
    cx_id_for_serp = config.get('GOOGLE_CX_ID_FOR_SERP_ANALYSIS')
    if not cx_id_for_serp:
        logger.warning("GOOGLE_CX_ID_FOR_SERP_ANALYSIS not configured. SERP data will be unavailable.")
        serp_data_str = None
    else:
