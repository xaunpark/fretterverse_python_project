...
def process_single_section_image(section_data, article_title, parent_section_name_for_subchapter,
    ...
    if not image_search_results:
        ...
        google_items = google_search(
            ...
            cx_id=config.get('GOOGLE_IMAGES_CX_ID'), # Cần thêm CX_ID cho Image Search vào config
            search_type='image',
            num_results=config.get('GOOGLE_SEARCH_NUM_RESULTS_IMAGES', 10), # Số lượng ảnh lấy về
            imgSize="large" # Từ workflow n8n của bạn
        )
    ...
    for attempt in range(max_selection_attempts):
        ...
        prompt_choose_image = image_prompts.CHOOSE_BEST_IMAGE_URL_PROMPT.format(
            article_title=article_title,
            section_context_string_for_selection=section_context_str_select,
            # section_name=s_name, # Đã có trong section_context_string_for_selection
