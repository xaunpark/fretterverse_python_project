...
def choose_author_for_topic(topic_or_keyword, author_personas_list_from_config, config):
    """
    Gọi LLM để chọn tác giả phù hợp nhất cho chủ đề/keyword.
    author_personas_list_from_config: List các dictionary author personas từ config.
    """
    logger.info(f"Choosing author for topic: '{topic_or_keyword}'")

    # --- Logic fallback author ---
    def get_default_author():
        default_author_id = str(config.get('DEFAULT_AUTHOR_ID', "1")) # Đảm bảo ID là string
        author_personas_cfg = config.get('AUTHOR_PERSONAS', [])
        # Tìm author mặc định trong list config, nếu không có thì tạo dict mặc định hoàn toàn
        default_author_dict = next(
            (author for author in author_personas_cfg if str(author.get("ID")) == default_author_id),
            {"name": "Default Author", "info": "Default author information for general topics.", "ID": default_author_id}
        )
        default_author_dict['ID'] = str(default_author_dict['ID']) # Đảm bảo ID là string
        logger.warning(f"Falling back to default author: {default_author_dict.get('name')} (ID: {default_author_dict.get('ID')})")
        return default_author_dict

    if not author_personas_list_from_config or not isinstance(author_personas_list_from_config, list):
        logger.error("Author personas list is empty, not a list, or not provided from config.")
        return get_default_author()

    try:
        # Chuyển list author personas thành chuỗi JSON để đưa vào prompt
        authors_list_json_string_for_format = json.dumps(author_personas_list_from_config)
    except TypeError as e:
        logger.error(f"Could not serialize author_personas_list_from_config to JSON: {e}", exc_info=True)
        return get_default_author()
    ...
    try:
        ...
        if chosen_author_data and isinstance(chosen_author_data, dict) and \
           'name' in chosen_author_data and 'ID' in chosen_author_data and 'info' in chosen_author_data: # Thêm check 'info'
            logger.info(f"Author chosen by LLM: {chosen_author_data.get('name')} (ID: {chosen_author_data.get('ID')})")
            chosen_author_data['ID'] = str(chosen_author_data.get('ID')) # Đảm bảo ID là string
            return chosen_author_data
        else:
            logger.error(f"LLM did not return valid author data. Response: {chosen_author_data}")
            return get_default_author()

    except Exception as e:
        logger.error(f"Error during author selection API call: {e}", exc_info=True)
        return get_default_author()
...
def get_serp_data_for_keyword(keyword, config):
    ...
    if not cx_id_shared:
        logger.error("GOOGLE_CX_ID not configured. Cannot fetch SERP data.")
        return None  
    try:
...
def check_keyword_uniqueness_and_upsert(keyword: str, 
                                        config: dict, 
                                        gsheet_handler, # GoogleSheetsHandler instance
                                        pinecone_handler, # PineconeHandler instance
                                        is_already_marked_unique: bool = False) -> bool:
    """
    Kiểm tra tính duy nhất của keyword dùng Pinecone.
    - Nếu 'is_already_marked_unique' là True, bỏ qua kiểm tra và upsert, trả về True.
    - Nếu unique, upsert vector vào Pinecone và cập nhật Google Sheet (chỉ cột 'Uniqe' = 'yes').
    - Nếu không unique, cập nhật Google Sheet (chỉ cột 'Uniqe' = 'no').
    - Nếu có lỗi, cập nhật Google Sheet (cột 'Uniqe' = 'error_...') và trả về False.
    Trả về True nếu keyword được coi là unique và xử lý thành công (hoặc đã unique từ trước).
    Trả về False nếu keyword không unique hoặc có lỗi nghiêm trọng.
    """
    logger.info(f"Checking uniqueness for keyword: '{keyword}'. Previously marked unique: {is_already_marked_unique}")
    
    gsheet_unique_col_name = config.get('GSHEET_UNIQUE_COLUMN') # Tên cột "Uniqe"

    if is_already_marked_unique:
        logger.info(f"Keyword '{keyword}' was previously marked as unique. Skipping Pinecone check and upsert.")
        return True # Đã unique từ trước, không cần làm gì thêm ở bước này

    # --- Các bước chỉ thực hiện nếu is_already_marked_unique là False ---
    if not pinecone_handler or not pinecone_handler.is_connected():
        logger.error("Pinecone handler not available or not connected. Cannot check uniqueness for new keyword.")
        # Không cập nhật GSheet ở đây vì đây là lỗi hệ thống, có thể thử lại sau.
        return False 

    # 1. Tạo embedding cho keyword
    logger.debug(f"Step 1.1: Generating embedding for keyword '{keyword}'")
    ...
    if not embedding_vector:
        logger.error(f"Failed to generate embedding for keyword '{keyword}'. Cannot determine uniqueness.")
        # Cập nhật GSheet với lỗi embedding
        if gsheet_handler and gsheet_unique_col_name:
            gsheet_handler.update_sheet_row_by_matching_column(
                config.get('GSHEET_SPREADSHEET_ID'), config.get('GSHEET_KEYWORD_SHEET_NAME'),
                config.get('GSHEET_KEYWORD_COLUMN'), keyword,
                {gsheet_unique_col_name: "error_embedding"}
            )
        return False

    # 2. Cắt giảm chiều và chuẩn hóa L2
    logger.debug("Step 1.2: Normalizing embedding vector.")
    dimension_to_cut = config.get('PINECONE_EMBEDDING_DIMENSION', 256)
    cut_dim_vector = embedding_vector[:dimension_to_cut]
    
    norm_sq = sum(x*x for x in cut_dim_vector)
    if norm_sq == 0: # Vector không hiếm khi xảy ra với text embedding
        normalized_embedding = cut_dim_vector 
        logger.warning(f"Embedding for '{keyword}' resulted in a zero vector after normalization (or was already zero).")
    else:
        norm = norm_sq**0.5
        normalized_embedding = [x / norm for x in cut_dim_vector]
    
    # 3. Query Pinecone để kiểm tra sự tồn tại/tương đồng
    logger.debug("Step 1.3: Querying Pinecone for similar vectors.")
    query_response = pinecone_handler.query_vectors(
        vector=normalized_embedding,
        top_k=1, # Chỉ cần top 1 để kiểm tra similarity
        include_values=False, 
        include_metadata=False 
    )

    is_determined_unique_by_pinecone = True # Giả định là unique ban đầu
    gsheet_unique_value_to_set = "error_pinecone_query" # Giá trị mặc định nếu query lỗi
    ...
    if query_response and query_response.matches:
        ...
        if score > config.get('PINECONE_SIMILARITY_THRESHOLD', 0.8):
            is_determined_unique_by_pinecone = False
            gsheet_unique_value_to_set = "no"
            logger.warning(f"Keyword '{keyword}' is NOT unique. Similar to '{top_match.id}' with score {score:.4f}.")
        else:
            # Score <= threshold -> unique
            gsheet_unique_value_to_set = "yes" 
            logger.info(f"Keyword '{keyword}' is unique (score {score:.4f} <= threshold).")
    elif query_response is None: # Có lỗi khi query Pinecone
        logger.error(f"Error during Pinecone query for '{keyword}'. Cannot determine uniqueness.")
        # Không cập nhật GSheet ở đây, để lần sau thử lại vì có thể là lỗi mạng
        return False 
    else: # query_response không có matches (index rỗng hoặc không tìm thấy gì gần)
        gsheet_unique_value_to_set = "yes"
        logger.info(f"No similar vectors found in Pinecone for '{keyword}'. Considered unique.")

    # 4. Upsert vào Pinecone nếu được xác định là unique bởi Pinecone check
    if is_determined_unique_by_pinecone: # Chỉ upsert nếu bước 3 xác định là unique
        logger.debug(f"Step 1.4: Upserting unique keyword '{keyword}' to Pinecone.")
        pinecone_id = normalize_keyword_for_pinecone_id(keyword)
        if not pinecone_id:
            logger.error(f"Generated empty Pinecone ID for keyword '{keyword}'. Cannot upsert.")
            gsheet_unique_value_to_set = "error_id_generation"
            is_determined_unique_by_pinecone = False # Đánh dấu lại là không thành công
        else:
            upsert_result = pinecone_handler.upsert_vectors(
                vectors_data=[{"id": pinecone_id, "values": normalized_embedding, "metadata": {"original_keyword": keyword}}]
            )
            if upsert_result and upsert_result.upserted_count > 0:
                ...
                # gsheet_unique_value_to_set vẫn là "yes"
            else:
                logger.error(f"Failed to upsert embedding for '{keyword}' to Pinecone. Response: {upsert_result}")
                gsheet_unique_value_to_set = "error_pinecone_upsert"
                is_determined_unique_by_pinecone = False # Đánh dấu lại là không thành công
    
    # 5. Cập nhật Google Sheet (chỉ cột "Uniqe")
    if gsheet_handler and gsheet_unique_col_name and gsheet_unique_value_to_set:
        logger.info(f"Step 1.5: Updating Google Sheet for keyword '{keyword}' with Unique status: '{gsheet_unique_value_to_set}'.")
        success_gsheet_update = gsheet_handler.update_sheet_row_by_matching_column(
            spreadsheet_id_or_url=config.get('GSHEET_SPREADSHEET_ID'),
            sheet_name_or_gid=config.get('GSHEET_KEYWORD_SHEET_NAME'), # Cập nhật sheet gốc
            match_column_header=config.get('GSHEET_KEYWORD_COLUMN'),
            match_value=keyword,
            data_to_update_dict={gsheet_unique_col_name: gsheet_unique_value_to_set} # Chỉ cập nhật cột này
        )
        if success_gsheet_update:
            logger.info(f"Google Sheet column '{gsheet_unique_col_name}' updated to '{gsheet_unique_value_to_set}' for keyword '{keyword}'.")
        else:
            logger.error(f"Failed to update Google Sheet for keyword '{keyword}' with Unique status.")
            # Nếu không update được GSheet, có thể vẫn coi là thành công nếu Pinecone đã xử lý
            # Hoặc có thể trả về False để báo hiệu lỗi. Tạm thời vẫn dựa vào is_determined_unique_by_pinecone
    
    return is_determined_unique_by_pinecone


def analyze_and_prepare_keyword(keyword_to_process: str, 
                                sheet_row_data: dict, 
                                config: dict, 
                                gsheet_handler_instance, # GoogleSheetsHandler instance
                                pinecone_handler_instance): # PineconeHandler instance
    """
    Bước 1 của quy trình: Phân tích keyword, kiểm tra tính phù hợp, duy nhất,
    chọn tác giả, lấy SERP và phân tích SERP.
    Cập nhật Google Sheet phù hợp nếu keyword bị dừng ở bước này.

    :param keyword_to_process: Chuỗi keyword cần xử lý.
    :param sheet_row_data: Dict chứa dữ liệu của hàng keyword từ Google Sheet.
    :param config: Dict cấu hình ứng dụng.
    :param gsheet_handler_instance: Instance của GoogleSheetsHandler.
    :param pinecone_handler_instance: Instance của PineconeHandler.
    :return: Dict chứa preparation_data nếu thành công, None nếu quy trình nên dừng.
    """
    preparation_data = {"original_keyword": keyword_to_process}
    logger.info(f"Starting Step 1: Analyze and Prepare Keyword for '{keyword_to_process}' using sheet data: {sheet_row_data}")

    gsheet_keyword_col = config.get('GSHEET_KEYWORD_COLUMN')
    gsheet_used_col = config.get('GSHEET_USED_COLUMN')
    gsheet_suitable_col = config.get('GSHEET_SUITABLE_COLUMN')
    gsheet_unique_col = config.get('GSHEET_UNIQUE_COLUMN')
    gsheet_id = config.get('GSHEET_SPREADSHEET_ID')
    gsheet_main_sheet_name = config.get('GSHEET_KEYWORD_SHEET_NAME') # Sheet gốc để cập nhật

    # --- 0. Kiểm tra tính phù hợp (Suitability) ---
    suitable_status_from_sheet = str(sheet_row_data.get(gsheet_suitable_col, '')).strip().lower()
    
    proceed_after_suitability = False
    if suitable_status_from_sheet == 'no':
        logger.info(f"Keyword '{keyword_to_process}' was previously marked as 'Suitable=no'. Stopping and ensuring 'Used=1'.")
        if gsheet_handler_instance:
            gsheet_handler_instance.update_sheet_row_by_matching_column(
                gsheet_id, gsheet_main_sheet_name,
                gsheet_keyword_col, keyword_to_process,
                {gsheet_used_col: "1", gsheet_suitable_col: "no"} # Cập nhật cả hai cho chắc chắn
            )
        return None
    elif suitable_status_from_sheet == 'yes':
        logger.info(f"Keyword '{keyword_to_process}' previously marked as 'Suitable=yes'. Proceeding.")
        preparation_data["is_suitable"] = True
        proceed_after_suitability = True
    else: # Trạng thái suitable chưa xác định ('', 'error_*', etc.) -> kiểm tra lại
        logger.info(f"Suitability status for '{keyword_to_process}' is '{suitable_status_from_sheet}'. Performing suitability check.")
        if check_keyword_suitability(keyword_to_process, config, gsheet_handler_instance): # Hàm này tự update GSheet nếu 'no'
            preparation_data["is_suitable"] = True
            proceed_after_suitability = True
            logger.info(f"Keyword '{keyword_to_process}' passed new suitability check.")
        else:
            logger.warning(f"Process stopped: Keyword '{keyword_to_process}' deemed not suitable after check. GSheet should be updated by check_keyword_suitability.")
            # check_keyword_suitability đã cập nhật Used=1, Suitable=no
            return None 
    
    if not proceed_after_suitability: # Double check
        return None

    # --- 1. Kiểm tra tính duy nhất (Uniqueness) và Upsert nếu cần ---
    unique_status_from_sheet = str(sheet_row_data.get(gsheet_unique_col, '')).strip().lower()
    keyword_is_considered_unique_for_processing = False

    if unique_status_from_sheet == 'yes':
        logger.info(f"Keyword '{keyword_to_process}' was previously marked as 'Uniqe=yes' in GSheet. Skipping Pinecone check, assuming already upserted.")
        keyword_is_considered_unique_for_processing = True
        # Gọi hàm check với is_already_marked_unique=True để nó không làm gì cả và trả về True
        # Điều này không thực sự cần thiết nếu chỉ kiểm tra flag, nhưng để giữ cấu trúc logic
        check_keyword_uniqueness_and_upsert( 
            keyword_to_process, config, gsheet_handler_instance, pinecone_handler_instance,
            is_already_marked_unique=True # Báo cho hàm con không cần check lại Pinecone
        )
    elif unique_status_from_sheet == 'no':
        logger.warning(f"Keyword '{keyword_to_process}' was previously marked as 'Uniqe=no' in GSheet. Stopping and ensuring 'Used=1'.")
        if gsheet_handler_instance:
            gsheet_handler_instance.update_sheet_row_by_matching_column(
                gsheet_id, gsheet_main_sheet_name,
                gsheet_keyword_col, keyword_to_process,
                {gsheet_used_col: "1", gsheet_unique_col: "no"} # Cập nhật cả hai
            )
        return None
    else: # Trạng thái unique chưa xác định ('', 'error_*', etc.) -> kiểm tra bằng Pinecone
        logger.info(f"Uniqueness status for '{keyword_to_process}' is '{unique_status_from_sheet}'. Performing Pinecone check and GSheet 'Uniqe' column update.")
        # Hàm này sẽ check Pinecone, upsert nếu unique, và cập nhật cột 'Uniqe' trên GSheet
        keyword_is_considered_unique_for_processing = check_keyword_uniqueness_and_upsert(
            keyword_to_process, config, gsheet_handler_instance, pinecone_handler_instance,
            is_already_marked_unique=False 
        ) 

    if not keyword_is_considered_unique_for_processing:
        logger.warning(f"Process stopped: Keyword '{keyword_to_process}' is not unique or error during uniqueness processing. GSheet 'Uniqe' column should be updated. Marking 'Used=1'.")
        # Nếu check_keyword_uniqueness_and_upsert trả về False (và nó không phải 'no' từ trước), 
        # GSheet cột Unique đã được cập nhật là no/error.
        # Giờ chỉ cần đảm bảo cột Used được cập nhật là '1'.
        if gsheet_handler_instance: # Luôn cập nhật Used=1 nếu quy trình dừng ở đây
             gsheet_handler_instance.update_sheet_row_by_matching_column(
                gsheet_id, gsheet_main_sheet_name,
                gsheet_keyword_col, keyword_to_process,
                {gsheet_used_col: "1"} # Cột Unique đã được hàm con xử lý
            )
        return None
    preparation_data["is_unique"] = True
    logger.info(f"Keyword '{keyword_to_process}' passed uniqueness processing.")

    # --- 2. Chọn tác giả ---
    author_personas_list = config.get('AUTHOR_PERSONAS', []) 
    if not author_personas_list:
         logger.warning("AUTHOR_PERSONAS not found or empty in config. Author selection will use default.")
         
    chosen_author = choose_author_for_topic(keyword_to_process, author_personas_list, config) 
    
    if not (chosen_author and 'ID' in chosen_author and chosen_author.get('ID') is not None):
        logger.critical(f"CRITICAL: Failed to obtain valid author data (even default) for keyword '{keyword_to_process}'.")
        # Nếu không có author, không thể tiếp tục. Đánh dấu keyword này là lỗi.
        if gsheet_handler_instance:
            gsheet_handler_instance.update_sheet_row_by_matching_column(
                gsheet_id, gsheet_main_sheet_name,
                gsheet_keyword_col, keyword_to_process,
                {gsheet_used_col: "1", gsheet_unique_col: sheet_row_data.get(gsheet_unique_col, "yes"), # Giữ lại trạng thái unique nếu đã có
                 config.get("GSHEET_STATUS_COLUMN", "Status"): "error_author_selection"} # Thêm cột Status (tùy chọn)
            )
        return None 
    preparation_data["chosen_author"] = chosen_author
    logger.info(f"Author for '{keyword_to_process}': {chosen_author.get('name')}, ID: {chosen_author.get('ID')}")

    # --- 3. Lấy SERP data ---
    cx_id_for_serp = config.get('GOOGLE_CX_ID')
    if not cx_id_for_serp:
        logger.warning("GOOGLE_CX_ID not configured. SERP data will be unavailable.")
        serp_data_str = None
    else:
        serp_data_str = get_serp_data_for_keyword(keyword_to_process, config) # Hàm này đã có cx_id
    
    if not serp_data_str:
        logger.warning(f"Could not fetch SERP data for '{keyword_to_process}'. Keyword analysis might be less accurate.")
    preparation_data["serp_data_string"] = serp_data_str 
    logger.info(f"SERP data fetched (or noted as absent) for '{keyword_to_process}'.")

    # --- 4. Phân tích SERP và Keyword ---
    # Ngay cả khi serp_data_str là None, vẫn gọi analyze_serp_and_keyword
    # Hàm analyze_serp_and_keyword cần xử lý trường hợp serp_data_string là None.
    keyword_analysis_result = analyze_serp_and_keyword(keyword_to_process, serp_data_str, config)
    if not keyword_analysis_result:
        logger.error(f"Failed to analyze keyword and SERP for '{keyword_to_process}'.")
        # Đánh dấu lỗi và dừng lại
        if gsheet_handler_instance:
            gsheet_handler_instance.update_sheet_row_by_matching_column(
                gsheet_id, gsheet_main_sheet_name,
                gsheet_keyword_col, keyword_to_process,
                {gsheet_used_col: "1", config.get("GSHEET_STATUS_COLUMN", "Status"): "error_serp_analysis"}
            )
        return None 
    preparation_data["keyword_analysis"] = keyword_analysis_result
    logger.info(f"Keyword analysis completed for '{keyword_to_process}'.")
...
def finalize_and_publish_article_step(
    ...
    author_id_for_post = article_meta.get('chosen_author_id', config.get('DEFAULT_AUTHOR_ID'))
    logger.info(f"Using Author ID for WordPress post: {author_id_for_post} (type: {type(author_id_for_post)})")
    if not author_id_for_post: # Nếu ID rỗng hoặc None
        logger.error("Author ID is missing or None. Cannot create WordPress post.")
    ...
    post_payload_for_function = {
        ...
        "categories_ids": [category_id_for_post] if category_id_for_post else [config.get('DEFAULT_CATEGORY_ID', 86)],
    ...
    if db_handler and post_id:
        ...
        # SỬA LOGIC TRÍCH XUẤT KEYWORDS Ở ĐÂY
        if ilj_keywords_response_raw and isinstance(ilj_keywords_response_raw, dict) and \
           "keywords" in ilj_keywords_response_raw and isinstance(ilj_keywords_response_raw["keywords"], list):
            actual_ilj_keywords_list = ilj_keywords_response_raw["keywords"]
            logger.info(f"Successfully extracted ILJ keywords list: {len(actual_ilj_keywords_list)} keywords.")
        elif ilj_keywords_response_raw and isinstance(ilj_keywords_response_raw, list): # Trường hợp LLM trả về list trực tiếp
            actual_ilj_keywords_list = ilj_keywords_response_raw
            logger.info(f"Successfully received ILJ keywords as direct list: {len(actual_ilj_keywords_list)} keywords.")
        else:
            logger.warning(f"Could not extract ILJ keywords list from LLM response for post ID {post_id}. Raw Response: {ilj_keywords_response_raw}")

        if actual_ilj_keywords_list: # Chỉ xử lý nếu có list keywords hợp lệ
...
def orchestrate_article_creation(keyword_to_process: str, 
                                 sheet_row_data_from_orchestrator: dict,
                                 config: dict, 
                                 gsheet_handler_instance: GoogleSheetsHandler, 
                                 pinecone_handler_instance: PineconeHandler, 
                                 db_handler_instance: MySQLHandler,
                                 unique_run_id_override: str = None):
    """
    ...
    preparation_results = analyze_and_prepare_keyword(
        ...
        sheet_row_data=sheet_row_data_from_orchestrator,
        config=config,
        gsheet_handler_instance=gsheet_handler_instance,
        pinecone_handler_instance=pinecone_handler_instance
    )
