...
def main_delete():
    ...
    parser.add_argument("--yes", "-y", action="store_true", help="Automatically confirm deletion without interactive prompt.")
    ...
    if keywords_to_remove:
        ...
        if args.yes: # Kiểm tra cờ --yes
            logger.info("Auto-confirmation enabled via --yes flag.")
        ...
        if proceed_with_deletion:
            logger.info(f"User confirmed or auto-confirmed. Proceeding with deletion from Pinecone index: {pinecone_handler.index_name} using keywords from GSheet ID: {gsheet_spreadsheet_id}, Sheet: {gsheet_delete_sheet_name}")
            ...
            logger.info("Deletion aborted by user or not auto-confirmed.")
    else:
