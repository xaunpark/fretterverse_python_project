...
Analyze the list of 10 Google search results for the keyword '{keyword}', including URLs, titles, and snippets, to determine the user's likely search intent, 
the most suitable content format, the appropriate article type, the writing model for a blog post, and the semantic keywords. Here are the search results:
...
Given the keyword '{keyword}', create a structured article outline in a valid JSON format, considering the search intent '{search_intent}', content format '{content_format}', article type '{article_type}', and integrating the '{selected_model}' writing model.

The outline should start with an 'Introduction' and end with 'FAQs' and 'Conclusion', with chapters and sub-chapters structured according to the roles in the '{selected_model}' model. 

Immediately following the introduction, generate a section highlighting top-rated products with sub-chapters for each product related to '{keyword}'. Ensure that all product names and models mentioned in the outline correspond to real, existing products in the market. IMPORTANT: Do not include fictional or hypothetical products. The article should reflect accurate and current market offerings, using only genuine product names and models that are verifiable and relevant to the topic.

For each sub-chapter in the "Top-Rated Products" section, the subchapterName must be the product's actual name. In addition, each product should have a unique headline in the 'headline' key that reflects a specific characteristic or benefit. The headline must follow the format "best for [specific use or feature]", clearly indicating why each product is the top choice within a certain range or for a specific use case. Do not repeat the product name in the headline.
...
Each product should have a unique headline that reflects a specific characteristic or benefit. The headline must follow the format "best for [specific use or feature]", clearly indicating why each product is the top choice within a certain range or for a specific use case. These headline should be inserted into the 'headline' key for each product. No need to repeat the product name in the headline.

After the top-rated products section, produce additional relevant chapters/sub-chapters that explore different facets of the product based on the keyword. Each of these chapters/sub-chapters should also include a "modelRole" corresponding to its purpose in the {selected_model} model and a 'separatedSemanticKeyword' list of carefully selected semantic keywords.
...
- 'chapters': An array of chapter objects, each with 'chapterName', 'modelRole', 'separatedSemanticKeyword', and 'length' key indicating the word count for the chapter. Include subchapters as needed, reflecting the selected content format and article type. Each subchapter is an object with 'subchapterName', 'modelRole', 'separatedSemanticKeyword', and 'length' key indicating the word count for the subchapter, aligning with its parent chapter. For the sub-chapters that are products belonging to top-rated products, there is an additional key 'headline'. If a chapter does not have subchapters, set 'subchapters' to an empty array ('[]') or 'null'.

Note: Avoid adding detailed content descriptions under each chapter or subchapter. Please maintain the exact spelling of variables as mentioned (subchapterName, chapterName, title, slug, description, chapters, modelRole, separatedSemanticKeyword, length). Exclude mentions of the year for a timeless appeal.
"""
...
I am organizing a database of articles and need to efficiently categorize them based on keywords and search intents. Below is a list of existing categories and sub-categories. Please analyze each provided keyword and its search intent, then recommend the most appropriate and specific category or sub-category for it. Provide only the most relevant and direct recommendation.

Task Instructions:

1. Understand the Keyword and Search Intent: Reflect on the meaning and the likely context of the keyword. How might a user think about this item or topic?
2. Recommend a Category/Sub-Category: Choose the most relevant and specific sub-category from the existing list. If a keyword fits well within a sub-category, recommend only that sub-category. Consider where a user would most directly find information about the keyword. When selecting a category from the list, ensure the name is used exactly as it appears, without alterations or spelling variations.
3. New Category Consideration: Only suggest a new category/sub-category if no existing one is a good fit. Ensure the new category is broad enough to include a range of related articles.
...
Consideration: "Evaluate whether the keyword fits more specifically within a sub-category. Recommend only the sub-category if it provides a clear and direct match for the keyword's intent."
...
Given that I have a website about music and musical instruments and need to write blog posts, evaluate whether the keyword '{keyword}' is suitable for the blog post.
