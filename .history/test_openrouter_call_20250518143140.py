...
try:
    from utils.config_loader import load_app_config  # Only import the function
    from utils.api_clients import call_openai_chat # Changed from call_llm_api
...
def main():
    ...
    # This will hold the configuration dictionary
    current_config = None

    try:
        # Load configuration (without site_name to use global .env by default)
        # Call load_app_config and get the returned configuration
        current_config = load_app_config() 
        logger.info("Configuration loaded.")
        
        # Use the 'current_config' dictionary
        openrouter_key = current_config.get("OPENROUTER_API_KEY")
        if not openrouter_key:
            logger.error("OPENROUTER_API_KEY not found in loaded config. The API call will likely fail.")
            # Depending on your logic, you might want to return if the key is crucial
            # return
        else:
        ...
        return # Stop execution if configuration loading fails

    # Ensure config was actually loaded
    if current_config is None:
        logger.error("Configuration object is None after loading attempt. Aborting API call.")
        return
    messages = [
    ...
    try:
        response_content = call_openai_chat( # Changed from call_llm_api
            messages=messages,
